...
- **批量任务队列**：拖拽或批量选择图片，支持勾选 / 全选 / 清空，任务列表可滚动，逐张显示进度条与状态
- **多线程并行**：可同时处理多张图片（最多 3 个豆包窗口），任务启动自动错开节奏
- **会话记忆**：每个任务独占一个豆包会话并记住会话 ID——重跑某张图、重启软件后，都会自动接回该图的历史对话继续处理，不会新建一堆会话；对话在网页端被删除后自动新建会话并更新记录
- **无图等待可调**：豆包回复结束后等待图片出现的秒数可在界面调整（默认 30 秒），出图慢时不再被误判"没有生成图片"
...
3. 勾选要处理的任务，点击"开始处理"。任务期间保持豆包窗口开启。
...
| 设置 | 位置 | 说明 |
| --- | --- | --- |
| 任务间隔 | 任务列表左上角 | 串行模式下每张图处理完成后等待的秒数；多线程模式自动错开节奏 |
| 多线程 | 任务列表左上角 | 开启后同时处理多张图片（最多 3 窗口并行）；频繁触发安全验证时建议关闭 |
| 无图等待 | 任务列表左上角 | 豆包回复结束后继续等待图片出现的秒数（5~300，默认 30）；出图慢被误判时调大 |
| 显示豆包窗口 | 右侧设置面板 | 调试时建议开启，稳定后可关闭转为后台运行 |
| 提示词与隔离带 | 右侧设置面板（齿轮） | 自定义提示词、隔离带位置（顶部/底部）、隔离带比例与白边补偿比例 |

...
- **无水印原图直取**：原图直发、不加工——通过 DevTools 协议旁路拦截豆包 chat/completion 接口的 SSE 流，读取服务端返回的 image_ori_raw 无水印原图（方案思路来自 doubao-no-watermark 项目）；拿到的是全尺寸原图，不加隔离带、不做裁切，直接导出
- **失败自动降级**：接口拦截不到无水印原图时，自动给原图加临时隔离带（让豆包 AI 标识落在带内）在同一会话重发一次，回到"页面候选 + 精确裁切"管线；隔离带位置（顶部/底部）、比例与白边补偿均可调
- **多重兜底下载**：候选大图下载失败时自动切换高清画布导出，尽量保证任务有结果
- **采集来源标记**：已完成的任务在文件信息行显示小标记——直取原图（接口拦截，未加工）/ 降级裁切（加隔离带重发）/ 页面采集（未加隔离带），悬停查看说明，降级频率一目了然

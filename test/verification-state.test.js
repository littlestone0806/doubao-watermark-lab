...
test('图形验证成功后追加的手机号验证：成功文案不否决，仍判定为验证中', () => {
  const result = state({
    candidates: [fakeElement({ tag: 'DIV', cls: 'verify-modal', text: '验证成功\n为了你的账号安全，请验证手机号\n获取验证码' })],
    composers: [fakeElement({ tag: 'TEXTAREA' })]
  });
  assert.equal(result.detected, true);
});

test('手机号验证界面出现在页面主体（非 verify 容器）时同样判定为验证中', () => {
  const result = state({
    composers: [fakeElement({ tag: 'TEXTAREA' })],
    bodyText: '为了你的账号安全，请输入手机号并获取短信验证码'
  });
  assert.equal(result.detected, true);
});

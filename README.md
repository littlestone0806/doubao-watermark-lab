...
- 🧠 **会话记忆 · 验证码自愈**：每张图独占会话、重启软件自动接回；多任务同时触发安全验证时只弹一个验证窗口，完成一次后所有被中断的任务自动重跑（支持图形验证后追加的手机号验证）
- 🖌 **手动涂抹重绘 · 自动质检**：残留水印涂抹后二次局部修复；保存后自动与原图逐像素对比，差异异常打黄标，预览支持"对比原图"与"差异热力"视图
- 🌐 **中英双语界面**：默认中文，一键切换 English，界面、任务进度、系统对话框与通知全部跟随，内置提示词同步切换语言
...
| 设置 | 位置 | 说明 |
| --- | --- | --- |
| 任务间隔 | 队列工具栏 | 串行模式下每张图处理完成后等待的秒数；多线程模式下不生效（任务全部同时进行） |
...
| 界面外观 | 右侧处理设置面板 | 深浅模式与主题色；点击调色盘按钮弹出气泡设置颜色 |
| 语言 Language | 右侧处理设置面板（界面外观内） | 中文 / English 一键切换（默认中文）；内置提示词会同步重置为对应语言版本 |
